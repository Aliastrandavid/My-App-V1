 ...
 * @return bool
 */
function is_slug_used($post_type, $slug, $exclude_id = '') {
    $posts = get_posts($post_type);
    
    foreach ($posts as $post) {
        if ($post['slug'] === $slug && $post['id'] !== $exclude_id) {
            return true;
        }
    }
 ...
 * @return string
 */
function get_post_preview_url($post_type, $post_id) {
    ...
    // Ensure we have a slug available
    $slug = isset($post['slug']) ? $post['slug'] : '';
    
    // Make sure config.php is included to ensure get_site_url is available
    if (!function_exists('get_site_url')) {
        // If get_site_url is not available, generate a simple URL
        $site_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://{$_SERVER['HTTP_HOST']}";
        return $site_url . '/' . $post_type . '/' . $slug . '?preview=true';
    }
    
    return get_site_url() . '/' . $post_type . '/' . $slug . '?preview=true';
}
